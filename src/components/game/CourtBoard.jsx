...
export async function buildOpts(game, teamKey, cardId, base, openModal, ui = {}) {
  ...
  if (cardId === 'dogged') {
    // A fatigued opponent, at any depth (2026-09-17) — what canPlay.js reads.
    const eligible = filterStarters(oppT.starters, (_, i) => getFatigue(game, oppKey, i) < 0);
    if (eligible.length === 0) { toast('No fatigued opponent on the floor.'); return null; }
    const idx = await pickFiltered(eligible, 'Target a fatigued opponent:', oppKey,
      (p, oi) => `(FAT ${getFatigue(game, oppKey, oi)}, ${getPS(game, oppKey, p.id)?.minutes || 0} min)`);
    if (idx === null) return null;
    opts.playerIdx = idx;
  }

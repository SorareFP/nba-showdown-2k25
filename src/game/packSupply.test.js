...
describe('guarantees still hold under supply', () => {
  ...
  it('keeps the legendary at the pack\'s own odds even when every super-rare is saturated', () => {
    // Since 2026-09-23 a Mega Deluxe carries a legendary at its apexOdds,
    // rolled once a pack. Supply decay weighs cards INSIDE a band; it must
    // not be able to push a slot across into the apex band. With every
    // super-rare saturated the legendary count is still the pack's, not more.
    ...
    for (let i = 0; i < N; i += 1) {
      ...
      const legs = rs.filter(r => r === 'legendary').length;
      expect(legs).toBeLessThanOrEqual(1);
      legendaries += legs;
    }
    const odds = PACK_TYPES.mega_deluxe.apexOdds;
    expect(legendaries / N).toBeLessThan(odds + 0.08);
    expect(legendaries / N).toBeGreaterThan(odds - 0.08);
  });
});

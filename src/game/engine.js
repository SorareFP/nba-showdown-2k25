...
//
// Until now the board printed "MUST REST" at sixteen minutes and nothing
// enforced it - not for the coach, not for a person - and the rulebook said
// the penalty "does not stop". This is the rule: a player with twelve minutes
// or more on the tracker sits the next section. The fourth quarter lifts it,
// and so does overtime, which is the last twelve minutes going long. Halftime
// wipes the tracker, so the count restarts for the second half.
//
// The tracker is the game's stamina, not a stopwatch: a rest takes four off,
// so "twelve on the tracker" is three net sections of load, which is what
// twelve straight minutes costs a body.
export const MAX_STRAIGHT_MINUTES = 12;

/** Whether the rule lifts for the section about to be played: Q4, and every overtime. */
export function restRuleLifted(g) {
  return Boolean(g?.overtime) || (g?.quarter ?? 1) >= 4;
}

/** Whether this player must sit the coming section. */
export function mustRest(g, teamKey, player) {
  if (!player || restRuleLifted(g)) return false;
  const ps = getPS(g, teamKey, player.id);
  return (ps?.minutes || 0) >= MAX_STRAIGHT_MINUTES;
}

/**
 * The players a coach may put on the floor from `pool`. The rule BENDS before
 * it breaks the game: if fewer than five are rested enough, the whole pool
 * stays eligible, least-tired first, so a five can always be named.
 */
export function pickablePool(g, teamKey, pool) {
  const rested = (pool || []).filter(p => !mustRest(g, teamKey, p));
  if (rested.length >= STARTERS) return rested;
  return [...(pool || [])].sort((a, b) => (getPS(g, teamKey, a.id)?.minutes || 0) - (getPS(g, teamKey, b.id)?.minutes || 0));
}

...
//   hidesBlankTier  whether the structural 1-2 blank row is printed
...
export const SETS = [
  {
    ...
    kind: 'season',
    editable: true,
    // The generator floors every chart with a blank 1-2 tier; the card does not
    // print it. See hidesBlankTier's own comment for why this is a set rule.
    hidesBlankTier: true,
    treatment: null,
  },
  {
 ...
 *
 * Now a lookup in SETS rather than a comparison against CURRENT_SET, so the two
...
export function hidesBlankTier(set) {
  return getSet(set)?.hidesBlankTier === true;
}

...
import { useCardPeek, PeekOverlay } from './CardPeek.jsx';
...
export function LightboxProvider({ children }) {
  ...
  // The hover magnifier (CardPeek.jsx): what is showing, and where the mouse
  // was when it came up. Never while the lightbox itself is open.
  const [peeked, setPeeked] = useState(null);  // { item, at }

  const open = useCallback((type, data) => { setPeeked(null); setItem({ type, data }); setFullRes(false); }, []);
  const close = useCallback(() => { setItem(null); setFullRes(false); }, []);
  const peek = useCallback((type, data, at) => { setPeeked(prev => (item ? prev : { item: { type, data }, at })); }, [item]);
  ...
  return (
    <LightboxCtx.Provider value={{ open, close, peek, unpeek }}>
      {children}
      {peeked && !item && <PeekOverlay item={peeked.item} at={peeked.at} onClose={unpeek} />}
      {item && (fullRes
